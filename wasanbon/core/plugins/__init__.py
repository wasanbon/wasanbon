...
def manifest(func):
    ...
    def wrapper__(*args, **kwds):
        ...
        self.parser.add_option('-a', '--alternative', help='print Alternatives of next argument (default=False)',
                               default=False, action='store_true', dest='alt_flag')
...
if -v is set, options.verbose_flag is set, which must be handled by developer of each plugin. """
    ...
    def get_manifest_functions(self, verbose=False, nocall=False):
        functions_dic = {}
    ...
    def get_function_names(self, verbose=False):
        return list(self.get_functions(verbose).keys())
...
class Loader():
    ...
    def __init__(self, directories, verbose=False):
        ...
        for name, dir in list(self._plugin_list.items()):
            if verbose:
                sys.stdout.write('# Loading %s plugin\n' % name)
    ...
    def get_manifest_plugin_names(self, package, nocall=False, verbose=False):
        """ Get Plugin Names in Package [admin|mgr] """
        return [p for p in dir(self._package[package]) if not p.startswith('_') and self.get_plugin(package, p).is_manifest_plugin()]
    ...
    def list_package_plugins(self, package, directory, verbose=False):
        ...
        for d in os.listdir(admin_dir):
            if d.endswith(self.ext):
                name = d[:-len(self.ext)]
                if verbose:
                    sys.stdout.write('# Plugin: %s Found.\n' % name)
    ...
    def load_plugin(self, name, directory, verbose=False):
        if verbose:
            sys.stdout.write('# Loading (%s) in %s\n' % (name, directory))
        ...
        try:
            spec = importlib.util.find_spec(os.path.basename(directory))
            ...
            traceback.print_exc()
            return None
        if getattr(m, 'admin', None) is None:
            setattr(m, 'admin', FunctionList())
        if getattr(m, 'mgr', None) is None:
            setattr(m, 'mgr', FunctionList())
        plugin = m.Plugin()
        plugin.__path__ = [directory]
        sys.path.pop(0)

        depends_plugin_names = plugin.depends()
        if type(depends_plugin_names) is not list:
            depends_plugin_names = [depends_plugin_names]
        for n in depends_plugin_names:
            if verbose:
                sys.stdout.write('# Plugin (%s) depends on %s.\n' % (name, n))
            if not n in list(self._plugin_list.keys()):
            ...
            if n.startswith('admin.'):
                if getattr(self._admin, n[6:], None) != None:
                    ...
                    if verbose:
                        sys.stdout.write('# Plugin %s is not loaded yet.\n' % n)
    ...
    def print_alternative(self, package, args):
        if len(args) >= 3:
            if args[2] == 'api':
                for n in self.get_manifest_plugin_names(package):
                    print(n, end=' ')
        else:
            print('list api')
    ...
    def run_command(self, package, subcommand, args):
        ...
        if args[2] == 'list':
            self.print_list_plugins(package, long_description)
            return 0
        elif args[2] == 'api':
            if len(args) < 4:
                self.print_help(package)
                return -1
            self.show_api(package, args[3], long_description)
    ...
    def print_list_plugins(self, package, long=False):
        ...
        """
        names = self.get_manifest_plugin_names(package)
        for name in names:
            print(' - %s:' % name)
    ...
    def show_api(self, package, plugin_name, long=False):
        ...
        functions = plugin.get_manifest_functions()
    ...
    def print_help(self, package):
        ...
        if package == 'admin':
            print("""This command is for plugin administration.

subcommands:
  list : |
    List plugins for package administration.
    $ wasanbon-admin.py plugin list
  api : |
    List APIs in the specified plugin.
    $ wasanbon-admin.py plugin api <PLUGIN_NAME>""")
...
subcommands:
  list : |
    List plugins for package management
    $ mgr.py plugin list
  api : |
    List APIs in the specified plugin.
    $ mgr.py plugin api <PLUGIN_NAME>""")
