...
class Plugin(PluginFunction):
    ...
    def launch_system(self, package,
        ...
        if not type(console_bind) is list:
    ...
    def is_standalone_rtc_launched(self, package, command, verbose=False):
        if verbose:
            sys.stdout.write('## Checking Standalone RTC (command=%s) is running or not.\n' % command)
        ...
        for proc in psutil.process_iter():
            for pid in pids:
                if proc.pid == pid:
                    ...
                    if verbose:
                        sys.stdout.write('### PID (%s) is running.\n' % pid)
                    return True
        if verbose:
    ...
    def terminate_standalone_rtcs(self, package, verbose=False):
        if verbose:
            sys.stdout.write('# Terminating Standalone RTCs.\n')
        retval = 0
        for command in package.standalone_rtc_commands:
    ...
    def terminate_standalone_rtc_with_command(self, package, command, verbose=False):
        ...
        for rtc in rtcs:
            ...
            if command.find(rtc_name) >= 0:
                if verbose:
                    sys.stdout.write('# Terminating RTC (%s)\n' % rtc_name)
                ...
                for proc in psutil.process_iter():
                    for pid in pids:
                        ...
                        if proc.pid == pid:
                            ...
                            for p in children:
                                if verbose:
                                    sys.stdout.write('## Kill Process (%d)\n' % p.pid)
                                try:
                                    p.kill()
    ...
    def launch_standalone_rtc(self, package, rtc, verbose=False, stdout=True):
        ...
        for command in commands:
            if verbose:
                sys.stdout.write('## Launch Standalone RTC (command=%s)\n' % command)

            rtc_name = rtc.rtcprofile.basicInfo.name
            if command.find(rtc_name) >= 0:
                if self.is_standalone_rtc_launched(package, command, verbose=verbose):
                    self.terminate_standalone_rtc(package, rtc, verbose=verbose)
                import subprocess
                if not stdout:
                    out = subprocess.PIPE
                else:
                    out = None
                proc = subprocess.Popen(command, stdout=out, stderr=out, shell=True)
    ...
    def terminate_rtcd(self, package, language, verbose=False):
        if verbose:
            sys.stdout.write('# Terminating rtcd for language (%s) in package (%s)\n' % (language, package.name))
    ...
    def launch_rtcd(self, package, language, rtcconf="", verbose=False):
        ...
        installed_rtcs = admin.systeminstaller.get_installed_rtc_names(package, language=language,
                                                                       verbose=verbose)
        if verbose:
            sys.stdout.write('# In Package %s, RTC (%s : lange=%s) are installed.\n' % (package.name, installed_rtcs, language))
        process = None
        if len(installed_rtcs) > 0:
            if verbose:
                sys.stdout.write('# Starting RTC-Daemon %s version.\n' % language)
            #from . import run
            from wasanbon.core.plugins.admin.systemlauncher_plugin import run
            process = start_rtcd(package, language, rtcconf, verbose)
            if verbose:
                sys.stdout.write('# Save rtcd_' + language + '_' + str(process.pid) + '\n')
        ...
        for lang in ['C++', 'Java', 'Python']:
            ...
            for mgr_addr in mgr_addrs[lang]:
                ...
                for i in range(0, try_count):
                    if verbose:
                        sys.stdout.write('## Getting Manager [%s]\n' % mgr_addr)
                    ...
                    try:
                        ...
                        break
                    except rts_exceptions.ZombieObjectError as e:
                        if i == try_count - 1:
                            sys.stdout.write('# ZombieObjectError Occured when exit_all_rtcs\n')
                            traceback.print_exc()
                            return -1
                    except rts_exceptions.NoSuchObjectError as e:
                        break
                if mgr:
                    if len(mgr.components) == 0:
                        if verbose:
                            sys.stdout.write('### Manager (%s) has no RTCs.\n' % mgr_addr)
                        cleaned = True
                    else:
                        for r in mgr.components:
                            if verbose:
                                sys.stdout.write('### Exitting RTC (%s)\n' % r.instance_name)
                            r.exit()

                        for i in range(0, try_count):
                            time.sleep(wait_time)
                            if verbose:
                                sys.stdout.write('## Getting Manager [%s] Again\n' % mgr_addr)
                            tree, mgr = get_manager(mgr_addr, full_path)
                            if len(mgr.components) == 0:
                                if verbose:
                                    sys.stdout.write('### Manager (%s) has no RTCs.\n' % mgr_addr)
                                ...
                                if verbose:
                                    sys.stdout.write('### Manager (%s) still have RTCs.\n' % mgr_addr)
                                pass
                    ...
                    if verbose:
                        sys.stdout.write('## Getting Manager failed.\n')
                    break
