...
class Plugin(PluginFunction):
    ...
    def to_dic(self, argv):
        ...
        self.parser.add_option('-d', '--detail', help='Detail information option. This shows the converted dictionary completely. A bit difficult to read. (default=False)',
                               default=False, action='store_true', dest='detail_flag')
    ...
    def show(self, argv):
        ...
        $ wasanbon-admin.py idl show <IDL_DATATYPE>
        """
        self.parser.add_option('-r', '--recursive', help='Recursive Type Parse. Show each members in inner structs. (default=False)',
                               default=False, action='store_true', dest='recursive_flag')
        options, argv = self.parse_args(argv[:], self._print_alternatives)
        verbose = options.verbose_flag  # This is default option
