...
class Plugin(PluginFunction):
    ...
    def create_rtcprofile(self, rtc, ns="localhost:2809", verbose=False):
        ...
        path, _ = rtctree.path.parse_path(full_path)
        if not path[-1]:
        ...
        keys = [key for key in list(comp.properties.keys()) if key.startswith('conf.default')]
        for key in keys:
            type = 'string'
        ...
        for sp_real in rtcp_real.serviceports:
            if sp.equals(sp_real):
                ...
                for i in sp.serviceInterfaces:
                    i_match_flag = False
                    if verbose:
                        sys.stdout.write('## Searching ServiceInterface %s :: %s (written in RTC.xml) ... ' % (i.name, i.type))
        ...
        for sp in rtcp.serviceports:
            if sp.equals(sp_real):
                ...
                for i_real in sp_real.serviceInterfaces:
                    i_match_flag = False
                    if verbose:
                        sys.stdout.write('## Searching ServiceInterface %s :: %s (implemented in RTC)' % (i_real.name, i_real.type))
                    for i in sp.serviceInterfaces:
                        if i.equals(i_real):
                            i_match_flag = True
                            if verbose:
