...
def build_rtc_cpp(rtcp, verbose=False):
    if verbose:
        sys.stdout.write('## Building C++ RT-component source\n')
    ...
    if sys.platform == 'win32':
        ...
        msbuild_dir = os.path.dirname(msbuild_path)
        os.environ['PATH'] = os.environ['PATH'] + ';' + msbuild_dir

    arg = []
    if sys.platform == 'linux':
        if verbose:
            print(' - Detected linux. modifying PKG_CONFIG_PATH environ.')
    ...
    if sys.platform == 'win32':
        ...
        if sln in os.listdir(os.getcwd()):
            ...
            cmd = [msbuild_path, sln, '/p:Configuration=Release']
            cmd += ['/clp:ErrorsOnly']
            #stdout = None if verbose else subprocess.PIPE
            #stderr = None if verbose else subprocess.PIPE
            ...
            sys.stdout.write(' - msbuild %s %s\n' % (os.path.basename(sln), '/p:Configuration=Release'))
            env = os.environ
...
def build_rtc_java(rtcp, verbose=False):
    ...
    cmd = [jarcmd,
           ...
           'cfe', os.path.join(bin_dir, rtc_name + '.jar'), rtc_name + 'Comp']
